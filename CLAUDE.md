...
- **Cross-file type clash on `Inter state route`, `Peak hour service`, `Social Oblicatory Route`**: unlike the within-file mixing above, these three columns are internally *consistent* per file but differ *across* files — e.g. `Inter state route` is 100% null in MTC (inferred as NullType/DoubleType there) but genuine strings (`'TN'`, `'KA'`...) in every other file; `Peak hour service` holds decimal-hour floats in SALEM (`9.55`, `12.0`) but text (`'NO'`, `'12hrs'`) elsewhere and is 100% null in 4 other files; `Social Oblicatory Route` is 100% null only in SETC. **Symptom**: when Bronze's `unionByName(allowMissingColumns=True)` tries to reconcile these mismatched per-file types, Spark widens to DOUBLE and throws `[CAST_INVALID_INPUT] The value 'NO' of the type "STRING" cannot be cast to "DOUBLE"`. **Resolution**: these 3 columns (plus `Route No`/`Service` above) are force-cast to pandas' nullable `string` dtype (`.astype("string")`, not just `str`) before `spark.createDataFrame()`, so Arrow always infers STRING for them in every file — including the all-null cases — before the union runs. See `FORCE_STRING_COLUMNS` in `notebooks/01_bronze/01_bronze_ingestion.py`.
